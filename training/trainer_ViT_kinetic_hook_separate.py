...
def trainer(model, train_dataloader, local_rank, device, optimizer, criterion, nepochs, nepochs_save, save_path, test_dataloader=None, kinetic_lambda = 0.0, writer = None):
    ...
    v_collect = [torch.torch.empty(0) for i in range(2*depth)]
    # save the norm of outputs of each layer
    x_norm_collect = [torch.torch.empty(0) for i in range(2*depth)]
    cosine_similarity = [torch.torch.empty(0) for i in range(2*depth)]
    def save_outputs_hook(layer_id):
        def fn(_, input, output):
            v_collect[layer_id] = output - input[0]
            x_norm_collect[layer_id] = torch.norm(output)
            cosine_similarity[layer_id] = torch.div(torch.einsum('ijk,ijk->',[input[0],output]), (torch.norm(output) * torch.norm(input[0])))
        return fn
...
def evaluation(model, dataloader, criterion, device, x_norm_collect, cosine_similarity, writer, epoch, nepochs_save):
    ...
    if (epoch+1) % nepochs_save == 0:  
        for i in range(depth):
            writer.add_scalar(f"attn: norm of outputs at iter {epoch}", x_norm[i],i)
            writer.add_scalar(f"attn: cosine similarity of input-outputs at iter {epoch}", cos_similarity[i],i)
            writer.add_scalar(f"MLP: norm of outputs at iter {epoch}", x_norm[i+depth],i)
            writer.add_scalar(f"MLP: cosine similarity of input-outputs at iter {epoch}", cos_similarity[i+depth],i)
