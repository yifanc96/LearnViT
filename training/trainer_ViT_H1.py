...
def trainer(model, train_dataloader, device, optimizer, criterion, nepochs, nepochs_save, save_path, test_dataloader=None, kinetic_lambda = 0.0, H1_lambda = 0.0):
    train_accs = []
    test_accs = [] if test_dataloader is not None else None
    
    for epoch in range(nepochs):
        running_loss, running_accuracy = train(model, train_dataloader, criterion, optimizer, device, kinetic_lambda = kinetic_lambda, H1_lambda = H1_lambda)
...
def train(model, dataloader, criterion, optimizer, device, kinetic_lambda = 0.0, H1_lambda = 0.0):
    '''
    ...
    for data, target in tqdm(dataloader):
        ...
        H1 = H1_lambda * sum([torch.mean((v_collect[i+1]-v_collect[i])**2) for i in range(len(v_collect)-1)])
