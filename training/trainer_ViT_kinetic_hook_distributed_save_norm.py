...
def trainer(model, train_dataloader, local_rank, device, optimizer, criterion, nepochs, nepochs_save, save_path, test_dataloader=None, kinetic_lambda = 0.0, writer = None):
    ...
    x_norm_collect = [torch.torch.empty(0) for i in range(depth)]
    ...
    for epoch in range(nepochs):
        ...
        if local_rank == 0: 
            ...
            writer.add_scalar('training/training accuracy', running_loss, epoch)

        if test_dataloader is not None and local_rank == 0:
            test_loss, test_accuracy = evaluation(model, test_dataloader, criterion, device, x_norm_collect, writer, epoch, nepochs_save)
...
def evaluation(model, dataloader, criterion, device, x_norm_collect, writer, epoch, nepochs_save):
    '''
    ...
    if (epoch+1) % nepochs_save == 0:  
        for i in range(depth):
            writer.add_scalar(f"norm of outputs at iter {epoch}", x_norm[i],i)
