...
### copied from https://github.com/skander-karkar/LAP/blob/master/dataloaders.py
...
def mean_and_std(datafolder = './data/dataset', batchsize = 300000, nworkers = 4):
...
def cifar10_dataloaders(batchsize, trainsize = 1, valsize = 0.5, testsize = 0.5, datafolder = './data/dataset/cifar10'):
	...
	testset = torchvision.datasets.CIFAR10(root = datafolder, train = False, download = True, transform = transforms.Compose(transform))
...
def cifar100_dataloaders(batchsize, trainsize = 1, valsize = 0.5, testsize = 0.5, datafolder = './data/dataset/cifar100'):
	datashape, mean, std, nclasses = (1, 3, 32, 32), (0.49139968, 0.48215841, 0.44653091), (0.24703223, 0.24348513, 0.26158784), 100
	transform = [transforms.ToTensor(), transforms.Normalize(mean, std)]
	data_aug = [transforms.RandomCrop(datashape[-1], padding = 4), transforms.RandomHorizontalFlip()]
	trainset = torchvision.datasets.CIFAR100(root = datafolder, train = True, download = True, transform = transforms.Compose(data_aug + transform))
	testset = torchvision.datasets.CIFAR100(root = datafolder, train = False, download = True, transform = transforms.Compose(transform))
	trainloader = get_subset_loader(trainset, batchsize, trainsize)
	valloader, testloader = get_subset_loaders(testset, batchsize, [valsize, testsize])
	return trainloader, valloader, testloader, datashape, nclasses, np.array(mean), np.array(std)

def mnist_dataloaders(batchsize, trainsize = 1, valsize = 0.5, testsize = 0.5, datafolder = './data/dataset/mnist'):
	datashape, mean, std, nclasses = (1, 1, 28, 28), (0.1306604762738429, ), (0.30810780717887876, ), 10
	transform = [transforms.ToTensor(), transforms.Normalize(mean, std)]
	trainset = torchvision.datasets.MNIST(root = datafolder, train = True, download = True, transform = transforms.Compose(transform))
	testset = torchvision.datasets.MNIST(root = datafolder, train = False, download = True, transform = transforms.Compose(transform))
	trainloader = get_subset_loader(trainset, batchsize, trainsize)
	valloader, testloader = get_subset_loaders(testset, batchsize, [valsize, testsize])
	return trainloader, valloader, testloader, datashape, nclasses, np.array(mean), np.array(std)

def imagenet2012_dataloaders(batchsize, trainsize = 0.98, valsize = 0.01, testsize = 0.01, datafolder = './data/dataset/imagenet2012'):
	...
	trainset = torchvision.datasets.ImageFolder(root = datafolder, transform = transforms.Compose(data_aug + transform))
	trainloader, valloader, testloader = get_subset_loaders(trainset, batchsize, [trainsize, valsize, testsize])
	return trainloader, valloader, testloader, datashape, nclasses, np.array(mean), np.array(std)

def imagenetdownloader_dataloaders(batchsize, trainsize = 0.98, valsize = 0.01, testsize = 0.01, datafolder = './data/dataset/imagenet-downloader'):
...
def tinyimagenet_dataloaders(batchsize, trainsize = 1, valsize = 0.5, testsize = 0.5, datafolder = './data/dataset/tinyimagenet'):
	datashape, mean, std, nclasses = (1, 3, 64, 64), [0.4802486, 0.44807222, 0.39754647], [0.2769859, 0.26906505, 0.2820814], 200
	transform = [transforms.ToTensor(), transforms.Normalize(mean, std)]
	data_aug = [transforms.RandomCrop(datashape[-1], padding = 4), transforms.RandomHorizontalFlip()]
	trainset = torchvision.datasets.ImageFolder(root = datafolder, transform = transforms.Compose(data_aug + transform))
	valset = torchvision.datasets.ImageFolder(root = datafolder, transform = transforms.Compose(transform))
	trainloader = get_subset_loader(trainset, batchsize, trainsize)
